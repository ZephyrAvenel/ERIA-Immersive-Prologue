...
test("PACK-003 contains vertical text-free placeholder illustrations", async () => {
  const imageDirectory = join(packRoot, "assets", "images");
  const images = (await readdir(imageDirectory)).filter((name) => name.endsWith(".svg"));
  assert.equal(images.length, 9);
  for (const image of images) {
    const source = await readFile(join(imageDirectory, image), "utf8");
    assert.equal(source.includes("<text"), false, image);
    assert.ok(source.includes("viewBox=\"0 0 900 1200\""), image);
  }
  const manifest = await readProjectJson(packRoot, "pack.json");
  assert.equal(manifest.coverImage, "assets/images/00-atlas.svg");
